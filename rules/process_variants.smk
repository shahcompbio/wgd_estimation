def get_maf_input_paths(wildcards):
    return {
        'consensus_somatic_maf': runinfo.paths[(wildcards.sample_id, 'WGS-SOMATICCALLING', 'consensus_somatic_maf')],
        'museq_paired_annotated': runinfo.paths[(wildcards.sample_id, 'WGS-SOMATICCALLING', 'museq_paired_annotated')],
        'strelka_snv_annotated': runinfo.paths[(wildcards.sample_id, 'WGS-SOMATICCALLING', 'strelka_snv_annotated')],
    }
...
rule filter_maf:
    ...
    output: os.path.join(config['intermediate_dir'], '{sample_id}_filtered.maf'),
    log: os.path.join(config['log_dir'], '{sample_id}_filtered.maf.log'),
...
rule maf_to_vcf:
    input:
        maf = os.path.join(config['intermediate_dir'], '{sample_id}_filtered.maf'),
    ...
    output:
        os.path.join(config['intermediate_dir'], '{sample_id}_filtered.vcf')
    log:
        os.path.join(config['log_dir'], '{sample_id}_filtered.vcf.log')
    params:
...
rule proc_vcf_for_mt:
    input:
        os.path.join(config['intermediate_dir'], '{sample_id}_filtered.vcf')
    output:
        os.path.join(config['results_dir'], '{sample_id}.vcf')
    log:
        os.path.join(config['log_dir'], '{sample_id}.vcf.log')
    singularity: 
...
rule proc_remixtpp_for_mt:
    ...
    log:
        os.path.join(config['log_dir'], '{sample_id}.cn.tsv.log')
    singularity: 
        "docker://soymintc/clickpdvcf:0.0.2"
    shell:
